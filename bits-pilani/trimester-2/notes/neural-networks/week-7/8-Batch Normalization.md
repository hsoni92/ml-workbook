...
- **Normalize** to approximately **zero mean** and **unit variance**. Use a small $\epsilon$ for **numerical stability** when dividing by the standard deviation:
...
- BN adds **learnable** scale $\gamma$ and shift $\beta$:
...
- **BN** stabilizes layer input statistics by **batch-wise** normalization, then recovers scaling and bias with $\gamma$ and $\beta$.
